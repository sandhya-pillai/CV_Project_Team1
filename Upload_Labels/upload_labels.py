...
if __name__ == '__main__':
    ...
    parser.add_argument("--api-key", help="api-key for labelbox")
    parser.add_argument("--project-id", help="project id to which the labels and videos are supposed to be uploaded")
    parser.add_argument("--ontology-id", default="", help="only if ontology is need, else ignore")
    parser.add_argument("--label-file", type=argparse.FileType('r'), help="local computer path to the json files with either a single label, or list of labels")
    parser.add_argument("--upload-labels", choices=["y", "n"], default="y", help="if y then provide label file too, else n")
    parser.add_argument("--mode-upload-video", choices=['single', 'batch', 'none'], help="select mode of video uploading, else none")
    parser.add_argument("--video-directory-path", default="", help="if mode single, then path is the file path of the video, else it is the directory which has all the videos to be uploaded")
    parser.add_argument("--video-dataset-name", default="video_demo_dataset", help="provide name of the dataset, default name is video_demo_dataset")
